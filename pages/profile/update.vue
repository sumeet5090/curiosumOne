            ...
            <card>
              <b-form @submit.prevent="onSubmit" @reset="onReset" id="my-form">
                <b-form-group id="form-displayname" label="Display Name:" label-for="form-displayname--input" description="We'll never share your email with anyone else.">
                  <b-form-input :readonly="user.infoSubmited" id="form-displayname--input" type="text" v-model="form.display_name"  placeholder="Enter email">
                  </b-form-input>
                </b-form-group>
                ...
                <b-form-group id="form-picture-url" label="Profile picture:" label-for="form-picture-url--input" description="Link to a new profile picture.">
                  <b-form-input :readonly="user.infoSubmited" id="form-picture-url--input" type="url" v-model="form.link_to_logo"  placeholder="https://example.website.com/images/demo.jpg">
                  </b-form-input>
                </b-form-group>
                <b-form-group id="form-bio" label="Bio:" label-for="form-bio--input" description="Talk about yourself.">
                  <b-form-textarea :readonly="user.infoSubmited" id="form-bio--input" type="text" v-model="form.bio" placeholder="Bio" :rows="4" max-rows="6" min-rows="3">
                  </b-form-textarea>
                </b-form-group>
                ...
                <b-form-group id="form-userType" label="Status:" label-for="form-userType--input" description="Select Student or Faculty.">
                  <div>
                    <b-form-select :disabled="user.infoSubmited"  @change="changeUserType()" :state="Boolean(form.userType)" id="form-userType--input"  v-model="form.userType">
                ...
                <b-form-group id="form-roleOnTeam" label="Role on Team:" label-for="form-roleOnTeam--input" description="Your role on team.">
                  <b-form-input :readonly="user.infoSubmited" :state="Boolean(form.roleOnTeam)" id="form-roleOnTeam--input" v-model="form.roleOnTeam" type="text" placeholder="Role on Team">
                  </b-form-input>
                </b-form-group>


                <b-form-group v-if="form.userType !== 'Faculty'" id="form-yearOfStudy" label="Current Year of Study:" label-for="form-yearOfStudy--input" description="Select Current Year of Study.">
                  <div>
                    <b-form-select :disabled="user.infoSubmited" :state="Boolean(form.yearOfStudy)" id="form-userType--input"  v-model="form.yearOfStudy">
                      <option :value="null">Current Year of Study</option>
                ...
                <b-form-group v-if="form.userType !== 'Faculty'" id="form-programOFStudy" label="Current Program of Study:" label-for="form-programOFStudy--input" >
                  <b-form-input :readonly="user.infoSubmited" :state="Boolean(form.programOFStudy)" id="form-programOFStudy--input" v-model="form.programOFStudy" type="text" placeholder="Current Program of Study">
                  </b-form-input>
                </b-form-group>
                ...
                <template v-if="form.eventOfParticipation == specialEvent">
                  ...
                  <template v-if="parseInt(form.formulaBharatDetails.age.value) < 18">
                    <b-form-group label="Signed Under 18 Waiver:" description="Upload signed Under 18 Waiver. [Max File size 200kB / .pdf, .jpg, .png]" >
                      ...
                      <!--                      v-model="/*form.formulaBharatDetails.age.file*/"-->
                    </b-form-group>
                  </template>
                  ...
                  <b-form-group id="form-ifDesignatedDriver.answer" label='If you are one of the designated drivers on the team, please select "Yes"' label-for="form-ifDesignatedDriver.answer--input" description="Are you one of the Designated Drivers for your team at the competition?">
                    <b-form-select  :disabled="user.infoSubmited" :state="(form.formulaBharatDetails.ifDesignatedDriver.answer == null)?false:true"   id="form-ifDesignatedDriver.answer--input" v-model="form.formulaBharatDetails.ifDesignatedDriver.answer">
                      <option :value="null">Please select an option</option>
                  ...
                  <b-form-group>
                    <b-form-checkbox
                      ...
                      unchecked-value=""
                    >
                      I accept the T&C (<a download="FORMULA BHARAT General Waiver - Portal.pdf" href="/pdf/FORMULA BHARAT General Waiver - Portal.pdf">Click here to Download T&C</a>)
                    </b-form-checkbox>
                  </b-form-group>
                </template>
...
<script>
    ...
    data() {
      return {
        ...
        csvFile: null,
        isAdmin: false,
        validate: ['userType', 'roleOnTeam', 'yearOfStudy', 'programOFStudy', 'eventOfParticipation','phone_parts'],
        isValid: true,
        form: {
          display_name: "",
          bio: "",
    ...
    methods: {
        ...mapActions(["getReq", "postReq", "putReq", "accessControl","uploadAndReturnFile","generatePdfPart", "generatePdfHead"]),
        ...
        async changeUserType() {
            setTimeout(() => {
                if(this.form.userType === 'Faculty') {
                    this.form.yearOfStudy = null
                    this.form.programOFStudy = null;
                    this.validate = this.validate.filter(valItem => valItem !== 'yearOfStudy' && valItem !== 'programOFStudy');
                } else {
                    this.validate = this.validate.concat(['yearOfStudy','programOFStudy']).filter( (value, index, self) => self.indexOf(value) === index)
                }
        ...
        generatePDF() {
          ...
          /*  const optisons = {
            ...
            const csvExporter = new ExportToCsv(optisons);
            ...
            let html =
             `<div class="pdf-wrapper">
                ...
                <table border="1">
                  ...
                  html += this.generatePdfPart(this.form.link_to_logo,'Profile picture', true, true, false);
                  html += this.generatePdfPart(this.form.bio,'Bio');
                  html += this.generatePdfPart(this.form.email,'Email');
                  html += this.generatePdfPart(this.form.userType,'Status');
                  html += this.generatePdfPart(this.form.roleOnTeam,'Role on Team');
                  html += this.generatePdfPart(this.form.yearOfStudy,'Current Year of Study');
            ...
            switch (type) {
                ...
                case 'PNG':
                    htmlToImage.toPng(element, {
                        ...
                        const link = document.createElement("a");
                        const fileName =`${this.form.display_name}.csv`;
        ...
        async loadFormData() {
          try {
            ...
            if (res.success) {
              ...
              if(this.user.role.includes('volunteer')) {
                  this.$router.push({name:'profile-volunteer-update'});
              }
